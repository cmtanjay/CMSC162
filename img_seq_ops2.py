...
def open_zip_file(self):
    ...
    if directory:
        ...
        frames = variables.image_paths
        show_frame(self, frames)
        show_frame_wrapper(self, frames)
...
def show_frame(self, frame_or_sequence=None):
    if frame_or_sequence is not None:
        if isinstance(frame_or_sequence, list):
            for frame_path in frame_or_sequence:
                img = Image.open(frame_path)
                display_single_frame(self, img)
        elif isinstance(frame_or_sequence, np.ndarray):
            # If frame_or_sequence is a single frame as a NumPy array
            display_single_frame(self, frame_or_sequence)
        elif isinstance(frame_or_sequence, str):
            # If frame_or_sequence is a string, assume it's an image path
            img = Image.open(frame_or_sequence)
            display_single_frame(self, img)
        else:
            # Handle other cases where frame_or_sequence is invalid
            reset_display(self)
    else:
        # Handle the case where frame_or_sequence is None
        # For example, when reset is called
        reset_display(self)
        
def show_frame_wrapper(self, frames):
    self.after(50, lambda: show_frame(self, frames))
...
def display_single_frame(self, frame):
    ...
    image = img_resize_aspectRatio(self, frame, available_width, available_height)
...
def reset_display(self):
    # Handle resetting the display when frame_or_sequence is None
...
def play_imgs(self):
    if not self.paused and self.current_frame < len(variables.image_paths) - 1:
        if not variables.is_filtered:
            show_frame_wrapper(self, variables.image_paths[self.current_frame])
        else:
            show_image(self, variables.img_seq[self.current_frame], " ")

        self.current_frame += 1
        self.after(50, lambda: play_imgs(self))  # Delay of 50 milliseconds
    elif self.current_frame == len(variables.image_paths) - 1:
        self.current_frame = 0
        show_frame_wrapper(self, variables.image_paths[self.current_frame])  # Display the first frame
